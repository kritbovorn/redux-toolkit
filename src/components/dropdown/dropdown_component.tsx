            ...
            {isShow
                ? <View style={[styles.dropdown, {maxHeight: sc.cardListHeight * 2.2}]}>
                    ...
                    {datas.map((item, index) =>
                        <TouchableHighlight key={index} underlayColor={'rgba(0, 0, 0, 0.1)'} onPress={() => onSelectedItem(item)} >
                            <Text key={index} style={[styles.textDropDown, gbs.body, {backgroundColor: item.name === title ? 'gold' : 'darkseagreen'}]}>{`${item.name}`}</Text>
                        </TouchableHighlight>
                    )}
                    </ScrollView>
